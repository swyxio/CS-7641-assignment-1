...
class DTLearner(BaseLearner):
    ...
    def prune(self):
        ...
        for candidate in reversed(candidates):  # Go backwards/leaves up
            if (
                tmp_tree.children_left[candidate] == tmp_tree.children_right[candidate]
            ):  # leaf node. Ignore
                continue
        ...
        assert (self._learner.tree_.children_left >= 0).sum() == (
            self._learner.tree_.children_right >= 0
        ).sum()
    ...
    def write_visualization(self, path):
        ...
        # print("-----------XXXXXXXX----------")
        # # print([x for x in dir(self._learner) if not x.startswith("_")])
        # # print([x for x in dir(dataset)])
        # # print([x for x in dir(clf)])
        # # print(dataset)
        # # print(clf)
        # print(dataset._details)
        # print("-----------XXXXXXXX----------")
        graph_data = tree.export_graphviz(
            self._learner,
            out_file=None,
            # feature_names=self._learner.classes_, #todo - actually give real names
            filled=True,
        )
