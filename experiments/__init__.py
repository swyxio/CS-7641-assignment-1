...
from .plot_learning_curve1 import plot_learning_curve as plc2
...
def basic_results(
    ...
    logger.info(
        "Computing basic results for {} ({} thread(s))".format(clf_type, threads)
    )
    ...
    if best_params:
        ...
        if curr_scorer == "f1":
            ...
            test_score = f1_score(test_y, y_pred, average="binary")
        else:
            y_pred = clf.predict(test_x)
        ...
        plt.savefig(
            "{}/images/{}_{}_NCM.png".format(OUTPUT_DIRECTORY, clf_type, dataset),
            format="png",
            dpi=150,
            bbox_inches="tight",
        )
        print(
            "CLF={}, dataset={}, test_score={}".format(
                clf_type, dataset_readable_name, test_score
            )
        )
        cv = clf
    else:
        # clf_gs = ms.GridSearchCV(estimator=clf, param_grid=params, n_jobs=threads, cv=5, scoring=curr_scorer)
        # clf_gs.fit(training_x, training_y)
        # plt = plot_model_complexity_curve(clf_gs.cv_results_, complexity_param['display_name'], params[complexity_param['name']], title="Decision tree")
        # plt.savefig('{}/images/{}_{}_MC.png'.format(OUTPUT_DIRECTORY, "DT", dataset), format='png', dpi=150)
        # print('Best param: best_params=' + repr(clf_gs.best_params_))
        #
        # # Check optimum training size
        ...
        #
        # plt = plc2(clf_gs.best_estimator_, 'Learning curve', training_x, training_y, cv=5, n_jobs=threads,
        #                               train_sizes=train_sizes, scoring=curr_scorer)
        # plt.savefig('{}/images/{}_{}_LC.png'.format(OUTPUT_DIRECTORY, "DT", dataset), format='png', dpi=150)
        ...
        reg_table.to_csv(
            "{}/{}_{}_reg.csv".format(OUTPUT_DIRECTORY, clf_type, dataset), index=False
        )
        ...
        best_params.to_csv(
            "{}/{}_{}_best_params.csv".format(OUTPUT_DIRECTORY, clf_type, dataset),
            index=False,
        )
        logger.info(" - Grid search complete")

        final_estimator.write_visualization(
            "{}/images/{}_{}_LC".format(OUTPUT_DIRECTORY, clf_type, dataset)
        )
        ...
        plt.savefig(
            "{}/images/{}_{}_CM.png".format(OUTPUT_DIRECTORY, clf_type, dataset),
            format="png",
            dpi=150,
            bbox_inches="tight",
        )
        ...
        with open("{}/test results.csv".format(OUTPUT_DIRECTORY), "a") as f:
            ...
            f.write(
                '"{}",{},{},{},"{}"\n'.format(
                    ts, clf_type, dataset, test_score, cv.best_params_
                )
            )

    n = training_y.shape[0]

    train_sizes = np.append(
        np.linspace(0.05, 0.1, 20, endpoint=False),
        np.linspace(0.1, 1, 20, endpoint=True),
    )
    ...
    curve_train_scores.to_csv(
        "{}/{}_{}_LC_train.csv".format(OUTPUT_DIRECTORY, clf_type, dataset)
    )
    curve_test_scores.to_csv(
        "{}/{}_{}_LC_test.csv".format(OUTPUT_DIRECTORY, clf_type, dataset)
    )
    ...
    plt.savefig(
        "{}/images/{}_{}_LC.png".format(OUTPUT_DIRECTORY, clf_type, dataset),
        format="png",
        dpi=150,
    )
...
def iteration_lc(
    ...
    reg_table = pd.DataFrame(cv.cv_results_)
    reg_table.to_csv(
        "{}/ITER_base_{}_{}.csv".format(OUTPUT_DIRECTORY, clf_type, dataset),
        index=False,
    )
    d = defaultdict(list)
    ...
    d = pd.DataFrame(d)
    d.to_csv(
        "{}/ITERtestSET_{}_{}.csv".format(OUTPUT_DIRECTORY, clf_type, dataset),
        index=False,
    )
    plt = plot_learning_curve(
        "{} - {} ({})".format(clf_type, dataset_readable_name, name),
    ...
    plt.savefig(
        "{}/images/{}_{}_ITER_LC.png".format(OUTPUT_DIRECTORY, clf_type, dataset),
        format="png",
        dpi=150,
    )
...
def make_timing_curve(
    x, y, clf, clf_name, dataset, dataset_readable_name, verbose=False, seed=42
):
    ...
    for i, frac in enumerate(sizes):
        for j in range(tests):
            ...
            x_train, x_test, y_train, y_test = ms.train_test_split(
                x, y, test_size=1 - frac, random_state=seed
            )
            st = clock()
    ...
    plt = plot_model_timing(
        "{} - {}".format(clf_name, dataset_readable_name),
        np.array(sizes) * 100,
        train_df,
        test_df,
    )
    plt.savefig(
        "{}/images/{}_{}_TC.png".format(OUTPUT_DIRECTORY, clf_name, dataset),
        format="png",
        dpi=150,
    )
...
def make_complexity_curve(
    ...
    curve_train_scores.to_csv(
        "{}/{}_{}_{}_MC_train.csv".format(
            OUTPUT_DIRECTORY, clf_name, dataset, param_name
        )
    )
    curve_test_scores.to_csv(
        "{}/{}_{}_{}_MC_test.csv".format(
            OUTPUT_DIRECTORY, clf_name, dataset, param_name
        )
    )
    plt = plot_model_complexity_curve(
        "Model Complexity: {} - {} ({})".format(
            clf_name, dataset_readable_name, param_display_name
        ),
        param_values,
        train_scores,
        test_scores,
...
def perform_experiment(
    ...
    ds_training_x, ds_training_y = ds.pre_training_adjustment(
        ds_training_x, ds_training_y
    )
    ...
    if not iteration_lc_only:
        ds_clf = basic_results(
            pipe,
            np.unique(ds.classes),
            ds_training_x,
            ds_training_y,
            ds_testing_x,
            ds_testing_y,
